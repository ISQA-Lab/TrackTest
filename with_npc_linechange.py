...
from common.linechange_common import generate_linechange
from scenarios.linechange_scenarios import build_linechange_scene
...
def func(
    ...
    scene = build_linechange_scene(seq, SCRIPT_DIR)

    config = dict(scene["config"])
    if start_road_idx is not None:
        config["init_road_idx"] = start_road_idx
        if len(scene["roads"]) == 2:
            config["dst_road_idx"] = 1 - start_road_idx

    return generate_linechange(
        uid=uid,
        save_dir=save_dir,
        seq=seq,
...
def main():
    parser = argparse.ArgumentParser(description="Unified linechange entry")
