...
def _scene_8(script_dir):
    from curves.curve_8 import roadline2, getderivative, on_upper_lane_relaxed, on_middle_lane_relaxed
    ...
    return {
        "seq": seq,
        "frame_start": frame_start,
        "roads": [on_upper_lane_relaxed, on_middle_lane_relaxed],
        "roadinfo": [[ego, back0], [back1]],
        "ego_idx": 0,
        "npcs": [
...
def _scene_13(script_dir):
    ...
    if direction == 0:
        init_idx, dst_idx = 1, 0
        init_point, dst_point = 0, 1
    else:
        init_idx, dst_idx = 0, 1
        init_point, dst_point = 1, 0
    ...
    return {
        ...
        "config": {
            ...
            "forward_axis": "y",
            "forward_sign": -1,
            "init_road_idx": init_idx,
            "dst_road_idx": dst_idx,
            ...
            "ref_line": roadline2,
            "plan_mode": "single_switch_with_none",
            "switch_min": 2,
            "path_name_template": "{seq}_linechange_{uid}",
        },
    }
...
def _scene_18(script_dir):
    ...
    start_lane = random.randint(0, 1)
    init_point = 1 if start_lane == 0 else 0
    dst_point = 0 if start_lane == 0 else 1
    ...
    return {
        ...
        "config": {
            ...
            "forward_axis": "x",
            "forward_sign": 1,
            "init_road_idx": start_lane,
            "dst_road_idx": 1 - start_lane,
            "init_point": init_point,
            "dst_point": dst_point,
            "speed_limit": 25,
            "seconds_range": (4, 5),
            "save_distance_range": (70, 100),
            "ref_line": roadline2,
            "plan_mode": "alternate_no_none",
            "alternate_min_stay": 2,
            "alternate_max_stay": 5,
            "transition_axis_rules": {
                "0->1": [("y", -1)],
                "1->0": [("y", 1)],
            },
...
def build_linechange_scene(seq, script_dir):
    if seq not in _SCENE_BUILDERS:
        raise ValueError(f"Unsupported linechange scene: {seq}")
