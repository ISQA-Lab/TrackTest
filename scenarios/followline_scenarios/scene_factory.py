...
def _scene_1(script_dir):
    ...
    return {
        ...
        "config": {
            "road_ref_lines": [roadline_middle, roadline_middle],
            "curve_mode": "x_of_y",
            "forward_axis": "y",
            "forward_sign": 1,
            "speed_limit": 28,
            "seconds_range": (4, 5),
            "save_distance_range": (45, 85),
            "follow_offset_range": (60, 120),
        },
    }
...
def _scene_2(script_dir):
    ...
    back0 = pickle.load(open(os.path.join(script_dir, "data/with_npc__15__generates__npc_0.pkl"), "rb"))[0:41]
    back1 = pickle.load(open(os.path.join(script_dir, "data/with_npc__15__generates__npc_1.pkl"), "rb"))[0:41]

    on_top = lambda x, y: And(y > getvalue(x, roadline2) + 15, y <= getvalue(x, roadline1) - 10)
    on_second = lambda x, y: And(y >= getvalue(x, roadline3), y < getvalue(x, roadline2) - 20)

    return {
        "seq": seq,
        "frame_start": frame_start,
        "roads": [on_top, on_second],
        ...
        "getderivative": getderivative,
        "config": {
            "road_ref_lines": [roadline2, roadline2],
            "curve_mode": "y_of_x",
            "forward_axis": "x",
            "forward_sign": 1,
            "speed_limit": 30,
            ...
            "follow_offset_range": (35, 80),
        },
    }


def _scene_3(script_dir):
    from curves.curve_3 import roadline1, roadline3, getderivative, on_upper_lane_relaxed, on_lower_lane_relaxed
    ...
    return {
        ...
        "getderivative": getderivative,
        "config": {
            "road_ref_lines": [roadline3, roadline1],
            "curve_mode": "y_of_x",
            "forward_axis": "x",
            "forward_sign": 1,
            "speed_limit": 30,
            "seconds_range": (4, 5),
            "save_distance_range": (30, 60),
            "follow_offset_range": (45, 95),
        },
    }


def _scene_4(script_dir):
    from curves.curve_4 import roadline2, roadline3, getderivative, on_upper_lane_relaxed, on_lower_lane_relaxed
    ...
    return {
        ...
        "getderivative": getderivative,
        "config": {
            "road_ref_lines": [roadline3, roadline2],
            "curve_mode": "y_of_x",
            "forward_axis": "x",
            "forward_sign": 1,
            "speed_limit": 30,
...
def _scene_5(script_dir):
    ...
    back0 = pickle.load(open(os.path.join(script_dir, "data/npc_5_linux__0__generates__npc_0.pkl"), "rb"))[0:50]
    back1 = pickle.load(open(os.path.join(script_dir, "data/npc_5_linux__0__generates__npc_1.pkl"), "rb"))[0:50]

    on_top = lambda x, y: And(y > getvalue(x, roadline2) + 15, y <= getvalue(x, roadline1) - 10)
    on_second = lambda x, y: And(y >= getvalue(x, roadline4), y < getvalue(x, roadline2) - 20)
    ...
    return {
        ...
        "config": {
            ...
            "speed_limit": 35,
            "seconds_range": (4, 8),
            "save_distance_range": (40, 70),
            "follow_offset_range": (50, 110),
        },
    }


def _scene_8(script_dir):
    from curves.curve_8 import roadline2, roadline3, roadline4, getderivative
    from curves.curve_8 import on_upper_lane_relaxed, on_middle_lane_relaxed, on_lower_lane_relaxed
    ...
    return {
        ...
        "getderivative": getderivative,
        "config": {
            "road_ref_lines": [roadline2, roadline3, roadline4],
            "curve_mode": "y_of_x",
            "forward_axis": "x",
            "forward_sign": -1,
            "speed_limit": 80,
            "seconds_range": (4, 5),
            "save_distance_range": (60, 120),
            "follow_offset_range": (90, 180),
        },
    }
...
def _scene_10(script_dir):
    ...
    return {
        ...
        "config": {
            ...
            "seconds_range": (3, 5),
            "save_distance_range": (30, 60),
            "follow_offset_range": (40, 90),
        },
    }


def _scene_13(script_dir):
    from curves.curve_13 import roadline2, roadline3, getderivative
    from curves.curve_13 import on_left_lane_relaxed, on_right_lane_relaxed
    ...
    return {
        ...
        "getderivative": getderivative,
        "config": {
            "road_ref_lines": [roadline2, roadline3],
            "curve_mode": "x_of_y",
            "forward_axis": "y",
            "forward_sign": -1,
            "speed_limit": 28,
...
def _scene_14(script_dir):
    ...
    ego = pickle.load(open(os.path.join(script_dir, "data/egos__14_ego_trajectory.pkl"), "rb"))[frame_start:frame_start + frame_len]
    back0 = pickle.load(open(os.path.join(script_dir, "data/npc_14_linux__0__generates__npc_0.pkl"), "rb"))[:frame_len]
    back1 = pickle.load(open(os.path.join(script_dir, "data/npc_14_linux__0__generates__npc_1.pkl"), "rb"))[:frame_len]
...
def _scene_18(script_dir):
    ...
    return {
        ...
        "getderivative": getderivative,
        "config": {
            "road_ref_lines": [roadline1, roadline2],
            "curve_mode": "y_of_x",
            "forward_axis": "x",
            "forward_sign": 1,
            "speed_limit": 30,
            "seconds_range": (4, 5),
            "save_distance_range": (25, 55),
            "follow_offset_range": (45, 100),
        },
    }
...
def build_followline_scene(seq, script_dir):
    if seq not in _SCENE_BUILDERS:
        raise ValueError(f"Unsupported followline scene: {seq}")
